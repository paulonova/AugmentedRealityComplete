...
public class MainActivity extends Activity {
	...
	class WebViewHandler extends WebViewClient {
		...
		public boolean shouldOverrideUrlLoading(WebView view, String url) {
			...
			if (!url.toLowerCase(Locale.US).startsWith("infocarproject")) {  // metaiosdkexample
		 ...
		 * 
		 * @param url
		 *            URL with prefix metaiosdkexample:// or
		 *            metaiosdkexamplearel://
		 */
		private void startTutorial(final String url) {
			...
			if (url.toLowerCase(Locale.US).startsWith("infocarproject://")) { // metaiosdkexample
				...
				try {
					final Class<?> activity = Class.forName(getPackageName() + ".Infocar" + tutorialId);
			...
			// else if
			// (url.toLowerCase(Locale.US).startsWith("metaiosdkexamplearel://"))
			// {
			//
			// final String arelConfigFile = "index.xml";
			// final File arelConfigFilePath =
			// AssetsManager.getAssetPathAsFile(getApplicationContext(),
			// "Infocar"+tutorialId+"/"+arelConfigFile);
			...
			// intent.putExtra(getPackageName()+ARELActivity.INTENT_EXTRA_AREL_SCENE,
			// arelConfigFilePath);
			// mLaunchingTutorial = true;
			// startActivity(intent);
			// }
			...
			// {
			// MetaioDebug.log(Log.ERROR,
			// "Invalid tutorial id, AREL configuration file not found: "+tutorialId);
